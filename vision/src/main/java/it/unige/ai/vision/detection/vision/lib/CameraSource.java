...
public class CameraSource {
    ...
    public static final int CAMERA_facingBack = CameraInfo
            .CAMERA_FACING_BACK;

    public static final int CAMERA_facingFront = CameraInfo
            .CAMERA_FACING_FRONT;

    private static final String TAG = CameraSource.class.getCanonicalName();

    /**
     * A dummy surface texture must be assigned a chosen name, but since we never use
     * an OpenGL context, we can choose any ID we want here. This is not a crazy hack - it is
     * actually how the camera team recommends using the camera without a preview.
     */
    private static final int DUMMY_textureName = 100;
     ...
     */
    private static final float TOLERANCE_aspectRatio = 0.01f;

    protected Activity mActivity;

    private Camera mCamera;

    protected int cameraFacing = CAMERA_facingBack;

    private Size previewSize;

    /**
     * Rotation of the device. See FrameMetadata#getRotation() for more.
     */
    ...
    private BaseProcessorInterface frameProcessor;

    /**
     * Map to convert between a byte array, received from the camera, and its associated byte buffer.
     * Byte buffers are used internally because this is a more efficient way to make calls
     * into native code later (avoids a potential copy).
     *
     * <p><b>Please note:</b> IdentityHashMap are used here instead of HashMap because the behaviour
     * of an array's equals, hashCode and toString methods is both useless and unexpected.
     * IdentityHashMap enforces identity ('==') check on the keys.
     */
    ...
    /**
     * Stops the camera and releases the resources of the camera and the underlying detector.
     */
    public void release() {
    ...
    public synchronized CameraSource start() throws IOException {
        ...
        dummySurfaceTexture = new SurfaceTexture(DUMMY_textureName);
    ...
    /**
     * Opens the camera and starts sending preview frames to the underlying detector with the supplied
     * surface holder used for the preview, thus frames can be displayed to the user.
     *
    ...
    public synchronized void stop() {
        ...
        if (processingThread != null) {
            try {
                // Wait for the thread to complete to ensure that we don't have multiple threads
        ...
        if (mCamera != null) {
            ...
            try {
                ...
                Log.e(TAG, "Camera preview not cleared: " + e);
            }
            mCamera.release();
            mCamera = null;
        }
        ...
        // since these will no longer be in use.
     ...
     */
    public synchronized void setFacing(int cameraFacing) {
        if ((cameraFacing != CAMERA_facingBack) && (cameraFacing != CAMERA_facingFront)) {
            throw new IllegalArgumentException("Invalid camera facing: " + cameraFacing);
        }
    ...
    /**
     * Returns the selected camera w.r.t. facing; one between {@link #CAMERA_facingBack}
     * and {@link #CAMERA_facingFront}.
     */
    ...
    private Camera createCamera() throws IOException {
        ...
        if (requestedCameraId == -1) {
            throw new IOException("Requested camera not found.");
        }
        Camera camera = Camera.open(requestedCameraId);

        SizePair sizePair = selectSizePair(camera, requestedPreviewWidth, requestedPreviewHeight);
        if (sizePair == null) {
            throw new IOException("Suitable preview size not found.");
        }
    ...
    private static List<SizePair> generateValidPreviewSizeList(Camera camera) {
        ...
        for (Camera.Size previewSize : supportedPreviewSizes) {
            ...
            for (Camera.Size pictureSize : supportedPictureSizes) {
                ...
                if (Math.abs(previewAspectRatio - pictureAspectRatio) < TOLERANCE_aspectRatio) {
    ...
    public void setMachineLearningFrameProcessor(BaseProcessorInterface processor) {
        synchronized (processorLock) {
            cleanScreen();
            if (frameProcessor != null) {
                frameProcessor.stop();
